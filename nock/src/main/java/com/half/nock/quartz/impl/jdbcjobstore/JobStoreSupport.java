 ...
 * @author James House
 */
public abstract class JobStoreSupport implements JobStore, NockJDBCConstants {
    ...
    private String semaphoreLockPrefix = DEFAULT_SEMAPHORE_LOCK_PREFIX;
    ...
    private int maxBatchSizeforGroupedTrigger = 60;
    ...
    /**
     * <p>
     * set the semaphore count , that should be used for group triggers.
     * default count is 1 and run as quartz before.
     * called reflectively
     * </p>
     *
     * @param count
    ...
    /**
     * <p>get the semaphore count , that should be used for group triggers.
     * default count is 1 and run as quartz before.
     * </p>
     */
    ...
    /**
     * <p>
     * get the semaphoreLockPrefix,任务分组的信号量前缀
     * </p>
     *
     * @return
     */
    public String getSemaphoreLockPrefix() {
        return this.semaphoreLockPrefix;
    }

    /**
     * <p>
     * 设置任务分组的信号量前缀,called reflectively
     * </p>
     *
     * @param semaphoreLockPrefix
     */
    public void setSemaphoreLockPrefix(String semaphoreLockPrefix) {
        this.semaphoreLockPrefix = semaphoreLockPrefix;
    }

    /**
     * get max batch size of triggers when using the job-grouped mode
     *
     * @return
     */
    public int getMaxBatchSizeforGroupedTrigger() {
        return maxBatchSizeforGroupedTrigger;
    }

    /**
     * get the max batch size of triggers when using the job-grouped mode
     * called reflectively
     *
     * @param maxBatchSizeforGroupedTrigger
     */
    public void setMaxBatchSizeforGroupedTrigger(int maxBatchSizeforGroupedTrigger) {
        this.maxBatchSizeforGroupedTrigger = maxBatchSizeforGroupedTrigger;
    }
     ...
     * </p>
     *
     * @see com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore
     */
    ...
    public void initialize(ClassLoadHelper loadHelper,
        ...
        if (getLockHandler() == null) {
            ...
            if (getUseDBLocks()) {
                ...
                triggerManager = new TriggerManager(new ConsistentHashDistribution<>());
                setLockHandler(new com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore(getTablePrefix(), getInstanceName(), getSelectWithLockSQL(), getSemaphoreCount(), getInstanceId(), getSemaphoreLockPrefix(), triggerManager, getClusterCheckinInterval()));
            } else {
                getLog().info(
    ...
    protected Connection getConnection() throws JobPersistenceException {
        ...
        try {
            if (!isDontSetAutoCommitFalse() && getSemaphoreCount() == 1) {
    ...
    /**
     * if using semaphore locks, we need to release thread lock,and still obtain the optimistic lock of DB.
     *
     * @param conn
     * @param lockName
     * @param doIt
    ...
    protected void releaseLock(Connection conn, String lockName, boolean doIt) {
        if (doIt && lockName.startsWith(getSemaphoreLockPrefix())) {
            ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).releaseSemaphoreLock();
        }
        releaseLock(lockName, doIt);
    }
    ...
    protected void recoverJobs(Connection conn) throws JobPersistenceException {
        try {
            ...
            List<OperableTrigger> recoveringJobTriggers = ((SemaphoreStdJDBCDelegate) getDelegate())
                    .selectTriggersForRecoveringJobs(conn, isUseSemaphoreLock());
            getLog()
                    .info(
                            "Recovering "
    ...
    protected RecoverMisfiredJobsResult recoverMisfiredJobs(
        ...
        boolean hasMoreMisfiredTriggers =
                ((SemaphoreStdJDBCDelegate) getDelegate()).hasMisfiredTriggersInState(
                        conn, STATE_WAITING, getMisfireTime(),
                        maxMisfiresToHandleAtATime, misfiredTriggers, isUseSemaphoreLock());
        ...
        for (TriggerKey triggerKey : misfiredTriggers) {
            ...
            getLog().info("Recover misfired trigger:" + trig.getKey());
    ...
    public void storeJobAndTrigger(final JobDetail newJob,
            ...
            throws JobPersistenceException {
        executeInLock(
                (isLockOnInsert()) ? isUseSemaphoreLock() ? null : LOCK_TRIGGER_ACCESS
                        : null,
                new VoidTransactionCallback() {
                    public void executeVoid(Connection conn) throws JobPersistenceException {
    ...
    public void storeCalendar(final String calName,
            ...
            throws JobPersistenceException {
        executeInLock(
                (isLockOnInsert() || updateTriggers) ? isUseSemaphoreLock() ? triggerManager.getCurrentSemaphoreLock() : LOCK_TRIGGER_ACCESS : null,
                new VoidTransactionCallback() {
                    public void executeVoid(Connection conn) throws JobPersistenceException {
    ...
    public List<OperableTrigger> acquireNextTriggers(final long noLaterThan, final int maxCount, final long timeWindow)
        ...
        if (isUseSemaphoreLock()) {
            //每次acquire的时候获取该实例对应的semaphore
            refreshTriggerSemaphores();
            lockName = triggerManager.getCurrentSemaphoreLock();
        }
        // getLog().info("acquireNextTriggers:1 lockName:" + lockName);
    ...
    protected List<OperableTrigger> acquireNextTrigger(Connection conn, long noLaterThan, int maxCount, long timeWindow)
        ...
        do {
            currentLoopCount++;
            try {
                List<TriggerKey> keys = ((SemaphoreStdJDBCDelegate) getDelegate()).selectTriggerToAcquire(conn, noLaterThan + timeWindow,
                        getMisfireTime(), maxCount, getMaxBatchSizeforGroupedTrigger(), isUseSemaphoreLock());
                ...
                for (TriggerKey triggerKey : keys) {
                    ...
                    int rowsUpdated = ((SemaphoreStdJDBCDelegate) getDelegate()).updateTriggerStateFromOtherStateWithTimestamp(conn, triggerKey, STATE_ACQUIRED, STATE_WAITING, nextTrigger.getNextFireTime().getTime());
                    if (rowsUpdated <= 0) {
                        continue; // next trigger
                    }
    ...
    public List<TriggerFiredResult> triggersFired(final List<OperableTrigger> triggers) throws JobPersistenceException {
        return executeInNonManagedTXLock(isUseSemaphoreLock() ? triggerManager.getCurrentSemaphoreLock() : LOCK_TRIGGER_ACCESS,
        ...
        synchronized (this) {
            if (null == delegate) {
                try {
                    ...
                    if (delegate.getClass() == SemaphoreStdJDBCDelegate.class) {
                        ((SemaphoreStdJDBCDelegate) delegate).initialize(getLog(), tablePrefix, instanceName, instanceId, getClassLoadHelper(), triggerManager);
                    } else {
    ...
    /**
     * refresh current semaphore lock
     *
     * @throws JobPersistenceException
     */
    protected void refreshTriggerSemaphores() throws JobPersistenceException {
        refreshTriggerSemaphores(null);
    }

    protected void refreshTriggerSemaphores(Connection conn) throws JobPersistenceException {
        if (conn == null) {
            //找到合适的任务节点
            executeInNonManagedTXLock(null,
                    conn1 -> {
                        try {
                            return ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).getSemaphoreLockForInstance(conn1);
                        } catch (SQLException e) {
            ...
            try {
                ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).getSemaphoreLockForInstance(conn);
            } catch (SQLException e) {
                e.printStackTrace();
            }
        }
    ...
    /**
     * 判断当前运行模式，trigger是否分组
     * todo:根据semaphores表加入判断，兼容老系统
     *
     * @return
     */
    protected boolean isUseSemaphoreLock() {
        return isUseSemaphoreLock(null);
    }

    protected boolean isUseSemaphoreLock(Connection conn) {
        if (semaphoreCount == 1) {
            return false;
        }
        if (triggerManager.getCurrentSemaphoreLock() == null) {
            try {
                if (conn == null) {
                    refreshTriggerSemaphores();
                } else {
                    refreshTriggerSemaphores(conn);
                }
            } catch (JobPersistenceException e) {
                e.printStackTrace();
            }
        }
        return triggerManager.getCurrentSemaphoreLock() != null;
    ...
    protected RecoverMisfiredJobsResult doRecoverMisfires() throws JobPersistenceException {
        ...
        String lockName = LOCK_TRIGGER_ACCESS;
        try {
            ...
            int misfireCount = (getDoubleCheckLockMisfireHandler()) ?
                    ((SemaphoreStdJDBCDelegate) getDelegate()).countMisfiredTriggersInState(
                            conn, STATE_WAITING, getMisfireTime(), isUseSemaphoreLock()) :
                    Integer.MAX_VALUE;
        ...
        try {
            ...
            if (firstCheckIn || (failedRecords.size() > 0)) {
                ...
                if (failedRecords.size() > 0) {
                    if (isUseSemaphoreLock(conn)) {
                        //release semaphore locks what belong to failed instances
                        commitConnection(conn);
                        conn.setAutoCommit(true);
                        releaseSemaphoreLockOfFailedRecords(conn, failedRecords);
                        lockName = triggerManager.getCurrentSemaphoreLock();
                    }
    ...
    /**
     * release wrong semaphore lock
     *
     * @throws JobPersistenceException
     */
    protected void doCheckSemaphores() throws JobPersistenceException {
        Connection conn = getNonManagedTXConnection();

        try {
            ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).releaseWrongSemaphoreLock(conn);
            releaseSemaphoreLockOfAbsentRecords(conn);
        } finally {
            cleanupConnection(conn);
        }
    ...
    /**
     * if the instance is offline，release the semaphore lock
     *
     * @param failedRecords
     */
    private void releaseSemaphoreLockOfFailedRecords(Connection conn, List<SchedulerStateRecord> failedRecords) {
        if (failedRecords == null || failedRecords.size() == 0) {
            return;
        }
        for (SchedulerStateRecord failedRecord : failedRecords) {

            //find failed semaphore locks
            List<TriggerSemaphore> failedSemaphores = ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).getSemaphoresByInstances(conn, failedRecord.getSchedulerInstanceId());

            if (failedSemaphores == null || failedSemaphores.size() == 0) {
                continue;
            }
            for (TriggerSemaphore failedSemaphore : failedSemaphores) {
                getLog().warn("find failed semaphore:" + failedSemaphore.getName());
                try {
                    //release semaphore locks
                    ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).updateSemaphoreWithInstance(conn, failedSemaphore.getName(), null, failedSemaphore.getCheckTime(), System.currentTimeMillis());
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }
    ...
    /**
     * if semaphore lock is obtained by absent instance ,release it.
     *
     * @param conn
     */
    private void releaseSemaphoreLockOfAbsentRecords(Connection conn) {
        List<TriggerSemaphore> semaphoreList = null;
        List<String> states = null;
        try {
            semaphoreList = ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).getSemaphores();
            states = ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).getSchedulerFromDB(conn);

        } catch (SQLException e) {
            e.printStackTrace();
        }
        if ((semaphoreList == null || semaphoreList.size() == 0) || (states == null || states.size() == 0)) {
            return;
        }

        for (TriggerSemaphore semaphoreLock : semaphoreList) {
            if (semaphoreLock.getInstanceName() == null || semaphoreLock.getInstanceName().equals("")) {
                continue;
            }
            if (!states.contains(semaphoreLock.getInstanceName())) {
                try {
                    //release semaphores
                    ((com.half.nock.quartz.impl.jdbcjobstore.StdRowLockSemaphore) getLockHandler()).updateSemaphoreWithInstance(conn, semaphoreLock.getName(), null, semaphoreLock.getCheckTime(), System.currentTimeMillis());
                } catch (SQLException e) {
                    e.printStackTrace();
                }
            }
     ...
     * 查找过期的semaphore
     */
    // protected List<TriggerSemaphore> findFailedSemaphores(Connection conn)
    //         throws JobPersistenceException {
    //     try {
    //         List<TriggerSemaphore> failedSemaphores = new LinkedList<>();
    //         long timeNow = System.currentTimeMillis();
    //
    //         List<TriggerSemaphore> semaphoreRecords = ((SemaphoreStdJDBCDelegate) getDelegate()).getSemaphoresFromDB(conn);
    //
    //         for (TriggerSemaphore rec : semaphoreRecords) {
    //             long lastChecktime = rec.getCheckTime() +
    //                     Math.max(5000L, (System.currentTimeMillis() - lastCheckin)) + 7500L;
    //             // find failed semaphores...
    //             if (rec.getInstanceName() != null && lastChecktime < timeNow) {
    //                 failedSemaphores.add(rec);
    //             }
    //         }
    //         return failedSemaphores;
    //     } catch (Exception e) {
    //         lastCheckin = System.currentTimeMillis();
    //         throw new JobPersistenceException("Failure identifying failed instances when checking-in: "
    //                 + e.getMessage(), e);
        ...
        if (failedInstances.size() > 0) {
            ...
            try {
                for (SchedulerStateRecord rec : failedInstances) {
                    ...
                    List<FiredTriggerRecord> firedTriggerRecs = ((SemaphoreStdJDBCDelegate) getDelegate())
                            .selectInstancesFiredTriggerRecords(conn, rec.getSchedulerInstanceId(), isUseSemaphoreLock());
    ...
    protected <T> T executeInNonManagedTXLock(
            String lockName,
            TransactionCallback<T> txCallback, final TransactionValidator<T> txValidator) throws
...
/////////////////////////////////////////////////////////////////////////////
    ...
    class ClusterManager extends Thread {
        ...
        private boolean manage() {
            ...
            try {
                //检查过期占用的semaphore乐观锁
                //检查state表的checkin时间来判断semaphore是否有效
                res = doCheckin();
                if (isUseSemaphoreLock()) {
                    doCheckSemaphores();
                }
